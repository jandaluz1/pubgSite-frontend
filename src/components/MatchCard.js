...
class MatchCard extends Component {
  render() {
    ...
    const team = rosters.filter(
      team =>
        team.relationships.participants.data.filter(
    ...
    return (
      <Paper className={classes.paper}>
        <Typography>
          {maps[match.data.attributes.mapName]} -{" "}
          {match.data.attributes.gameMode.replace("-", " ")}
        </Typography>
        <Grid container direction="row" alignItems="flex-end">
          <Grid item className={classes.item}>
            <Typography variant='subtitle1'>Rank: </Typography>
            <Typography inline variant="h5">
              {team.attributes.stats.rank} / {rosters.length}
            </Typography>
          </Grid>
          <Grid item className={classes.item}>
          ...
          <Grid item className={classes.item}>
            <Typography>DBNO:</Typography>
            <Typography variant="subtitle1">
              {player.attributes.stats.DBNOs}
            </Typography>
          </Grid>
          <Grid item className={classes.item}>
            <Typography>Damage:</Typography>
            <Typography variant="subtitle1">
              {Math.floor(player.attributes.stats.damageDealt)}
            </Typography>
          </Grid>
          <Grid item className={classes.item}>
            <Typography>Time Alive:</Typography>
            <Typography variant="subtitle1">
              {Math.floor(player.attributes.stats.timeSurvived / 60)}:
              {Math.floor(player.attributes.stats.timeSurvived % 60) < 10
                ? "0" + Math.floor(player.attributes.stats.timeSurvived % 60)
                : Math.floor(player.attributes.stats.timeSurvived % 60)}
            </Typography>
          </Grid>
        </Grid>
